...
unit_test_coverage: ## Run all unit tests in the pkg folder and get test coverage
	...
	go test -coverprofile=coverage.txt ./pkg/... && go tool cover -func=coverage.txt
...
lint_docker: ## Run docker instance of linting across the code base
	docker run --rm -v $(PWD):/app -v ~/.cache/golangci-lint/v2.6.2:/root/.cache -w /app golangci/golangci-lint:v2.6.2 golangci-lint run ./...
